...
SPI_HandleTypeDef* ICM42688_SPI_Handle = &hspi2;
...
#define SPI_IMU_CS_Port GPIOB
#define	SPI_IMU_CS_Pin  GPIO_PIN_9

//SPI 读写一个字节
//TxData:要写入的字节
//返回值:读取到的字节
uint8_t IMU_SPI_ReadWriteByte(uint8_t TxData)
{
    uint8_t Rxdata;
    HAL_SPI_TransmitReceive(ICM42688_SPI_Handle, &TxData, &Rxdata, 1, 100);       
...
static void Icm_Spi_ReadWriteNbytes(uint8_t* pBuffer, uint8_t len)
{
    ...
    for(i = 0; i < len; i ++)
    {
		*pBuffer = IMU_SPI_ReadWriteByte(*pBuffer);
        pBuffer++;
    }
...
#if defined(ICM_USE_HARD_SPI)
    HAL_GPIO_WritePin(SPI_IMU_CS_Port,SPI_IMU_CS_Pin,GPIO_PIN_RESET);
    reg |= 0x80;
    /* 写入要读的寄存器地址 */
    IMU_SPI_ReadWriteByte(reg);
    /* 读取寄存器数据 */
    regval = IMU_SPI_ReadWriteByte(0xFF);
    HAL_GPIO_WritePin(SPI_IMU_CS_Port,SPI_IMU_CS_Pin,GPIO_PIN_SET);
#elif defined(ICM_USE_I2C)
...
static void icm42688_read_regs(uint8_t reg, uint8_t* buf, uint16_t len)
{
    ...
    while(len)
	{
		*buf = IMU_SPI_ReadWriteByte(0x00);
		len--;
		buf++;
	}
    HAL_GPIO_WritePin(SPI_IMU_CS_Port,SPI_IMU_CS_Pin,GPIO_PIN_SET);
...
//        icm42688_write_reg(ICM42688_INT_SOURCE0, reg_val);
		icm42688_write_reg(ICM42688_INT_CONFIG, 0x30);
        icm42688_write_reg(ICM42688_INT_SOURCE0, 0x08); // 配置INT1 DRDY INT
...
{ 
    ...
    // //PA4
    // GPIO_Initure.Pin=SPI_IMU_CS_Pin;            //PA4
    // GPIO_Initure.Mode=GPIO_MODE_OUTPUT_PP;  //推挽输出
    // GPIO_Initure.Pull=GPIO_PULLUP;          //上拉
    // GPIO_Initure.Speed=GPIO_SPEED_HIGH;     //快速         
    // HAL_GPIO_Init(SPI_IMU_CS_Port,&GPIO_Initure);     //初始化
    
	// HAL_GPIO_WritePin(SPI_IMU_CS_Port,SPI_IMU_CS_Pin,GPIO_PIN_SET);			                //SPI FLASH不选中
	// SPI1_Init();		   			        //初始化SPI
